...
FILLER = "the "
TASK = "Reply with OK."
USAGE = "usage: cold_prefill_ab.py <arm> [reps=3] [out.json=<script dir>/cold_prefill_<arm>.json]"
...
def main() -> int:
    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help"):
        ...
        return 2
    global ARM, REPS, OUT_JSON
    ARM = sys.argv[1]
    REPS = int(sys.argv[2]) if len(sys.argv) > 2 else 3
    OUT_JSON = (
        Path(sys.argv[3])
        if len(sys.argv) > 3
        else Path(__file__).resolve().parent / f"cold_prefill_{ARM}.json"
    )
    ...
    for i, (name, target, timeout) in [(i, r) for i, r in enumerate(LADDER) for _ in range(REPS)]:
        salt = unique_salt()
        ...
        if i == 0 and eightk_user is None:
